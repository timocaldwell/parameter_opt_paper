...
%\textbf{Potential examples:
%\begin{enumerate}
%\item Stiffness identification of rod with variable stiffness (example a plant).
%\item Wire connected at two ends. (Initial idea is designing a wire's variable stiffness to minimize strain at connectors...)
%\item Calculating best stiffness profile of a mesh to reduce normal displacement to a point force at a specific location (thinking of a bullet proof vest--could be relevant to your amorphous materials...).  Not sure how TREP will work with a mesh.  It prefers tree structures but handles loops with holonomic constraints.  In other words, I don't know how well it'll scale with a large number of holonomic constraint.  Will execution time scale well? Will holonomic constraint stay valid?
%\item Anything else? Maybe something that isn't optimizing over spring constants...
%\end{enumerate}
%}
...
%key points:
%\begin{itemize}
%\item Parameter -- a feature of the system that is constant during the evolution of the system.
%--- for example beam stiffnesses, moments of inertia, lengths, 
%\item Parameter Estimation of mechanical systems
%\item Difficult to estimate parameters when the system has simple, uniform geometries -- e.g. estimating stiffness of plant organisms or wires connected at both ends (loops in the geometry), 
%--- for instance, systems where Euler-Bernoulli or Timoshenko beam equations are not clearly applicable.
%\item useful for motion planning and control.
%\item uses variational integrator approach which has nice properties like conserving energy or .... (LOOK THIS UP!)
%--- forms a discrete-time mechanical system formulation.
%\item paper provides an efficient discrete-time parameter gradient
%--- gradient is useful for optimal parameter estimation
%--- estimation can be done with respect to trajectory information
%\item Using TREP for efficient calculation
%--- gradient calculation relies on system linearizations.  A standard approach to calculate the linearization is to find the symbolic equations of motion and take the symbolic partial derivatives with respect to the state variables.  Due to chain and product rule, the number of state variables can grow exponentially with the number of connected components.  TREP instead uses a tree-based representation to make these calculations scale.  Note derivatives are not approximations.
%--- Can handle dissipation and holonomic constraints
%--- Make argument (similar to Johnson's in his scalability paper) that we use variational integrators because we want energy dissipation to be a parameter of the system that can be estimated and not compromised by the choice of numerical integrator.
%\end{itemize}

%\textbf{NOTE THIS HAS TO BE CLEANED UP QUITE A BIT.  MOSTLY THE ``WHY IS THIS HARD'' SUBSECTION NEEDS TO BE INTEGRATED INTO HERE AND SHOULD TALK ABOUT BAXTER AND MENTION ITS USEFULNESS TO MANIPULATION...}
...
We model the loop as six links forming a hexagon.  The links connect together through spherical joints attached with tosional springs.  The goal of the example is to identify the torsional springs' spring constants.   Alternatively, other parameters could be identified like lengths, masses, and damping coefficients.  The presented approach also extends to identification of the coefficient of friction of a system's interaction with a surface similar to \textbf{cite Miller}.  %  A holonomic constraint is needed to constrain the loop's beginning and end frames to the same position and orientation in order to ``close the loop''.  
